...
public:
	...
	*//**
	...
	string encodeAMEFObject(AMEFObject packet) throws AMEFEncodeException
	{
		string dat = encodeSingleAMEFPacket(packet);
		int l = dat.length();
		return (dat);
		//return dat;
	}
	...
	/*string encodeSingleAMEFPacket(AMEFObject packet) throws AMEFEncodeException
	{
		...
		{
			buffer.append(encodeSingleAMEFPacket(pack));
		}
	...
	*//**
	...
	string encodeSinglePacketB(AMEFObject *packet,bool ignoreName)
	{
		...
		if(!ignoreName)
		{
			...
			{
				string lengthb = intTocharArrayWI(packet->isObject()?packet->getNamedLength():packet->getLength());
