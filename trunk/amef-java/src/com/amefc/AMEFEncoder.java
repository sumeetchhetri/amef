...
public final class AMEFEncoder
{
	 ...
	 */
	/*public char[] encode(AMEFObject packet,boolean ignoreName) throws AMEFEncodeException
	{
		String dat = encodeSinglePacket(packet,ignoreName);
		int l = dat.length();		
		return (AMEFResources.intToByteArrayS(l, 4) + dat).toCharArray();
	}	*/
	
	public String encodeB(AMEFObject packet,boolean ignoreName) throws AMEFEncodeException
	{
		String dat = "";
		String enc = encodeSinglePacketB(packet, ignoreName);
		char[] len = AMEFResources.intToByteArray(enc.length(), 4);
		dat += String.copyValueOf(len) + enc;
		return dat;
	}
	
	public String encodeWL(AMEFObject packet,boolean ignoreName) throws AMEFEncodeException
	{
	...
	/*public char[] encodeWL(AMEFObject packet,boolean ignoreName) throws AMEFEncodeException
	{
		String dat = encodeSinglePacket(packet,ignoreName);
		int l = dat.length();		
	...
	*//**
	...
	private char[] encodeSinglePacketB(AMEFObject packet,boolean ignoreName) throws AMEFEncodeException
	{		
		...
		if(packet.getPackets().size()==0)
		{
			buffer.write(packet.getValue().toCharArray());
		}
		String retval = "";
		if(!ignoreName)
			retval +=  packet.getName();
	...
	public String getPacketValue(String value) throws AMEFEncodeException
	{
		String arr = "";
		char type;
		if(value.length()<=256)
		...
		{
			arr +=(char)type;
			char[] len = AMEFResources.intToByteArray(value.length(), 1);
			System.arraycopy(len, 0, arr, 1, len.length);
			System.arraycopy(value.toCharArray(), 0, arr, len.length+1, value.toCharArray().length);
		}
		else if(type==AMEFObject.STRING_65536_TYPE)
		{
			arr +=(char)type;
			char[] len = AMEFResources.intToByteArray(value.length(), 2);
			System.arraycopy(len, 0, arr, 1, len.length);
			System.arraycopy(value.toCharArray(), 0, arr, len.length, value.toCharArray().length);
		}
		else if(type==AMEFObject.STRING_16777216_TYPE)
		{
			arr +=(char)type;
			char[] len = AMEFResources.intToByteArray(value.length(), 3);
			System.arraycopy(len, 0, arr, 1, len.length);
			System.arraycopy(value.toCharArray(), 0, arr, len.length, value.toCharArray().length);
		}
		else if(type==AMEFObject.STRING_TYPE)
		{
			arr +=(char)type;
			char[] len = AMEFResources.intToByteArray(value.length(), 4);
			System.arraycopy(len, 0, arr, 1, len.length);
			System.arraycopy(value.toCharArray(), 0, arr, len.length, value.toCharArray().length);
		}
		else if(type==AMEFObject.BOOLEAN_TYPE || type==AMEFObject.CHAR_TYPE 
		...
		else if(type==AMEFObject.OBJECT_TYPE)
		{
			...
			if(length<256)
			{
				arr +=(char)AMEFObject.VS_OBJECT_TYPE;
				char[] len = AMEFResources.intToByteArray(value.length(), 1);
				System.arraycopy(len, 0, arr, 1, len.length);
				System.arraycopy(value.toCharArray(), 0, arr, len.length, value.toCharArray().length);
			}
			...
			{
				arr +=(char)AMEFObject.S_OBJECT_TYPE;
				char[] len = AMEFResources.intToByteArray(value.length(), 2);
				System.arraycopy(len, 0, arr, 1, len.length);
			}
			else if(length<16777216)
			{
				arr +=(char)AMEFObject.B_OBJECT_TYPE;
				char[] len = AMEFResources.intToByteArray(value.length(),3);
				System.arraycopy(len, 0, arr, 1, len.length);
			}
			else
			{
				arr +=(char)AMEFObject.OBJECT_TYPE;
				char[] len = AMEFResources.intToByteArray(value.length(),4);
				System.arraycopy(len, 0, arr, 1, len.length);
			}
		}
	...
	public String getPacketValue(int integer) throws AMEFEncodeException
	{
		char type;
		int ind = 1;
		if(integer<256)
		...
		String arr = "";
		arr +=(char)type;
		char[] len = AMEFResources.intToByteArray(integer, ind);
		System.arraycopy(len, 0, arr, 1, len.length);
		return arr;
	}
	
	public String getPacketValue(long lon) throws AMEFEncodeException
	{
		char type;
		int ind = 1;
		if(lon<256)
		...
		String arr = "";
		arr +=(char)type;
		char[] len = AMEFResources.longToByteArray(lon, ind);
		System.arraycopy(len, 0, arr, 1, len.length);
		return arr;
	}
	
	public char[] getPacketValue(double lon) throws AMEFEncodeException
	{
		StringBuilder buffer = new StringBuilder();
	...
	public char[] getPacketValue(float lon) throws AMEFEncodeException
	{
		StringBuilder buffer = new StringBuilder();
		char type = AMEFObject.DOUBLE_FLOAT_TYPE;
		getValue(String.valueOf(lon),type, buffer);
	...
	public char[] getPacketValue(boolean lon) throws AMEFEncodeException
	{
		return (AMEFObject.BOOLEAN_TYPE+(lon?"1":"0")).toCharArray();
	}
	
	public char[] getPacketValue(char lon) throws AMEFEncodeException
	{
		return (AMEFObject.CHAR_TYPE+""+lon).toCharArray();
	}
	...
	//---------------------------------------------------------------------------------------//
	private String encodeSinglePacketB(AMEFObject packet,boolean ignoreName) throws AMEFEncodeException
	{
		String buffer = "";
		int length = packet.getNamedLength(ignoreName);
		int pos = 0;
		if(!ignoreName)
		{
			if(packet.getType()==AMEFObject.OBJECT_TYPE)
			{
				if(length+4<256)
				{
					buffer +=AMEFObject.VS_OBJECT_TYPE;
				}
				else if(length+4<65536)
				{
					buffer +=AMEFObject.S_OBJECT_TYPE;
				}
				else if(length+4<16777216)
				{
					buffer +=AMEFObject.B_OBJECT_TYPE;
				}
			}
			else
				buffer +=(char)packet.getType();
			buffer +=(char)',';
			buffer += String.copyValueOf(packet.getName());
			...
			{
				char[] lengthb = AMEFResources.intToByteArrayWI(packet.getlength());
				buffer += String.copyValueOf(lengthb);
		...
		if(packet.getPackets().size()==0 && packet.getType()!=AMEFObject.NULL_STRING && packet.getType()!=AMEFObject.NULL_BOOL
				...
				&& packet.getType()!=AMEFObject.NULL_NUMBER)
		{
			buffer += String.copyValueOf(packet.getValue());
		}
		return buffer;
	}
}
