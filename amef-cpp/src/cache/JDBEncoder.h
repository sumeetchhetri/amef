...
#ifndef JDBENCODER_H_
#define JDBENCODER_H_
...
class JDBEncoder
{
	 ...
	 * encode the JDBObject to the charstream for wire transmission
	 */
	/*char* encode(JDBObject packet,boolean ignoreName) throws AMEFEncodeException
	{
...
public:
	...
	string encodeB(JDBObject* packet,bool ignoreName)
	{
	...
	string encodeWL(JDBObject* packet,bool ignoreName)
	{
	...
	/*char* encodeWL(JDBObject packet,boolean ignoreName) throws AMEFEncodeException
	{
		string dat = encodeSinglePacket(packet,ignoreName);
		int l = dat.length();
		return dat.getchars();
	...
	*//**
	 ...
	 * encode the JDBObject to the charstream for wire transmission
	 *//*
	string encodeS(JDBObject packet,boolean ignoreName) throws AMEFEncodeException
	{
		string dat = null;
		if(ignoreName)
	...
	string encodeJDBObject(JDBObject packet) throws AMEFEncodeException
	{
	...
	string getFinalVal(char type,string buffer,int length,string delim, string name)
	{
		...
		else if(type==JDBObject::OBJECT_TYPE)
		{
			if(length<256)
			{
				retval = JDBObject::VS_OBJECT_TYPE + delim + name + delim + intTocharArrayS(length, 1) + buffer;
			}
			else if(length<65536)
			{
				retval = JDBObject::S_OBJECT_TYPE + delim + name + delim + intTocharArrayS(length, 2) + buffer;
			}
			else if(length<16777216)
			{
				retval = JDBObject::B_OBJECT_TYPE + delim + name + delim + intTocharArrayS(length, 3) + buffer;
			}
			else
			{
				retval = JDBObject::OBJECT_TYPE + delim + name + delim + intTocharArrayS(length, 4) + buffer;
			}
		}
	...
	/*string encodeSingleJDBPacket(JDBObject packet) throws AMEFEncodeException
	{
	...
	*//**
	 ...
	 *//*
	string encodeSinglePacket(JDBObject packet,boolean ignoreName) throws AMEFEncodeException
	{
		stringBuilder buffer = new stringBuilder();
		if(packet==null)
		{
	...
	char* encodeSinglePacketB(JDBObject packet,boolean ignoreName) throws AMEFEncodeException
	{
	...
	string getPacketValue(string value)
	{
		...
		else if(type==JDBObject::OBJECT_TYPE)
		{
			...
			if(length<256)
			{
				arr[0] = (char)JDBObject::VS_OBJECT_TYPE;
				string len = intTocharArray(value.length(), 1);
				arr.append(len);arr.append(value);
			}
			else if(length<65536)
			{
				arr[0] = (char)JDBObject::S_OBJECT_TYPE;
				string len = intTocharArray(value.length(), 2);
				arr.append(len);
			}
			else if(length<16777216)
			{
				arr[0] = (char)JDBObject::B_OBJECT_TYPE;
				string len = intTocharArray(value.length(),3);
				arr.append(len);
			}
			else
			{
				arr[0] = (char)JDBObject::OBJECT_TYPE;
				string len = intTocharArray(value.length(),4);
				arr.append(len);
			}
		}
	...
	{
		return (JDBObject::BOOLEAN_TYPE+(lon?"1":"0"));
	}

	string getPacketValue(char lon)
	{
		return (JDBObject::CHAR_TYPE+""+lon);
	}
	...
	//---------------------------------------------------------------------------------------//
	string encodeSinglePacketB(JDBObject *packet,bool ignoreName)
	{
		...
		if(!ignoreName)
		{
			if(packet->getType()=='o')
			{
				...
				{
					buffer.push_back(JDBObject::VS_OBJECT_TYPE);
				}
				else if(bufferlength+4<65536)
				{
					buffer.push_back(JDBObject::S_OBJECT_TYPE);
				}
				else if(bufferlength+4<16777216)
				{
					buffer.push_back(JDBObject::B_OBJECT_TYPE);
				}
			}
			else
		...
		{
			JDBObject *pack = packet->getPackets().at(i);
